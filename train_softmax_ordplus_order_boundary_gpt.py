...
from roi_ordinal_baselines_common import pairwise_order_regularization
...
class ResNet18SoftmaxOrdPlusOrderBoundary(nn.Module):
    def __init__(
        self,
...
def main(a):
    ...
    model = ResNet18SoftmaxOrdPlusOrderBoundary(
        pretrained=True,
    ...
    for epoch in range(a.epochs):
        ...
        for imgs, labels in tr_loader:
            ...
            l_order_ce = ce_cls_fn(logits_ord, labels)
            l_order_reg = pairwise_order_regularization(out["feat"], labels)
            l_ord = l_order_ce + a.order_weight * l_order_reg
...
if __name__ == "__main__":
    ...
    ap.add_argument("--order_weight", type=float, default=0.1)
    ...
    ap.add_argument("--out", type=str, default="softmax_ordplus_order_boundary.pt")
    main(ap.parse_args())
