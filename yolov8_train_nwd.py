...
if __name__ == '__main__':
    ...
    # 开始训练 - 使用NWD损失函数（基于Wasserstein距离的小目标优化）
    results = model.train(
        ...
        save_period=30,
        box_loss='nwd',  # 使用NWD损失函数，基于Wasserstein距离优化小目标检测
        project='runs/train_nwd',  # 保存训练结果的项目文件夹
        name='yolov8n_nwd_smallbox',  # 训练任务名称
        exist_ok=True  # 允许覆盖已存在的训练结果
    )
    
    print("\n" + "="*60)
    print("训练完成！使用了NWD损失函数优化小目标检测")
    print("="*60)