...
class ResNet18DCHOrdinal(nn.Module):
    def __init__(self, pretrained=True, num_classes=4, fusion_mode="fixed", init_alpha=0.7):
        super().__init__()
    ...
    def forward(self, x):
        ...
        return coral_logits, cls_logits, feat
...
def compute_losses(
    ...
    soft_cum = torch.stack([p_ge1_s, p_ge3_s, p_ge5_s], dim=1)

    loss_cons = F.binary_cross_entropy(coral_probs, soft_cum.detach())
    ...
    loss_fuse = F.binary_cross_entropy(fused, levels)

    total = loss_coral + lambda_ce * loss_ce + lambda_cons * loss_cons + lambda_mono * loss_mono + lambda_fuse * loss_fuse
    return total, loss_coral, loss_ce, loss_cons, loss_mono, loss_fuse


@torch.no_grad()
def evaluate(model, loader, device, alpha=0.7, lambda_ce=0.5, lambda_cons=0.2, lambda_mono=0.05, lambda_fuse=0.0):
    model.eval()
    losses = []
    y_true, y_pred = [], []
    for imgs, labels in loader:
        ...
        total, *_ = compute_losses(model, coral_logits, cls_logits, feat, labels, lambda_ce, lambda_cons, lambda_mono, lambda_fuse)
...
def main(args):
    ...
    eff_lambda_fuse = args.lambda_fuse
    if args.fusion_mode != "fixed" and eff_lambda_fuse <= 0:
        eff_lambda_fuse = 1.0
    ...
    for epoch in range(args.epochs):
        ...
        for imgs, labels in tr_loader:
            ...
            labels = labels.to(device)
            coral_logits, cls_logits, feat = model(imgs)
            total, _, _, _, _, _ = compute_losses(
                model,
                coral_logits,
                cls_logits,
                feat,
...
if __name__ == "__main__":
    ...
    ap.add_argument("--fusion_mode", type=str, default="fixed", choices=["fixed", "static", "dynamic"])
