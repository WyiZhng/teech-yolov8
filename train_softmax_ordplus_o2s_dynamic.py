...
class GatingNetwork(nn.Module):
    def __init__(self, input_dim=524, hidden_dim=128, num_classes=4):
        super().__init__()
...
class ResNet18SoftmaxOrdPlusDynamic(nn.Module):
    def __init__(self, pretrained=True, ord2seq_d_model=256, ord2seq_layers=2):
        ...
        # Gating input: feat(512) + p_s(4) + p_o(4) + H_s(1) + H_o(1) + M_s(1) + M_o(1) = 524
        self.gating = GatingNetwork(input_dim=feat_dim + 4 + 4 + 1 + 1 + 1 + 1)
    ...
    def forward(self, x, labels=None):
        ...
        alpha = self.gating(gating_input) # (B, 4)
...
def main(a):
    ...
    model = ResNet18SoftmaxOrdPlusDynamic(pretrained=True).to(device)
    ...
    for epoch in range(a.epochs):
        ...
        for imgs, labels in tr_loader:
            ...
            p_fuse, p_soft, p_ord = out["p_fuse"], out["p_soft"], out["p_ord"]
            
            l_ce = ce_loss(out["logits_cls"], labels)
            l_ord = out["loss_ord2seq"]
            l_emd = emd_loss(p_fuse, labels)
            l_cons = F.l1_loss(p_soft, p_ord.detach())
            
            # Loss weighting for Class 2 samples specifically in EMD
            if a.class_2_boost > 1.0:
                is_c2 = (labels == 2).float()
                sample_weights = 1.0 + (a.class_2_boost - 1.0) * is_c2
                # Custom weighted EMD (simplified)
                onehot = F.one_hot(labels, 4).float()
                l_emd = (((torch.cumsum(p_fuse, 1) - torch.cumsum(onehot, 1))**2).mean(1) * sample_weights).mean()

            loss = a.l_ce * l_ce + a.l_ord * l_ord + a.l_emd * l_emd + a.l_cons * l_cons
...
if __name__ == "__main__":
    ...
    ap.add_argument("--class_2_boost", type=float, default=1.5, help="Boost loss for Class 2 (B)")
    ...
    ap.add_argument("--out", type=str, default="softmax_ordplus_o2s_dynamic.pt")
    main(ap.parse_args())
