# tools/score_proposals_with_icdas4.py
...
def main(a):
    ...
    with torch.no_grad():
        batch=[]; paths=[]; boxes=[]
        def flush():
            nonlocal p1s,p3s,p5s,batch,paths,boxes
            if not batch: return
            x=torch.stack(batch).to(device)
            z=torch.sigmoid(model(x)).cpu().numpy()
            if z.shape[1]==3:
                p1,p3,p5 = z[:,0], z[:,1], z[:,2]
            else:
                p1,p3,p5 = z[:,0], z[:,2], z[:,4]
            p1s += p1.tolist(); p3s += p3.tolist(); p5s += p5.tolist()
            batch=[]; paths=[]; boxes=[]
...
if __name__=='__main__':
    ...
    ap.add_argument('--pred_csv', required=True)   # 候选csv（如 val_predictions_top400_scored.csv）
