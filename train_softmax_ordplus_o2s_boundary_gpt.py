...
class ResNet18SoftmaxOrdPlusBoundaryV2(nn.Module):
    """
    关键优化点
    1. 保留 dynamic OGAF 的全局动态融合
    2. 加入 0/A candidate mask，限制边界专家介入范围
    3. 0/A expert 重点学习 hard 0/A samples
    4. 最终输出 p_final 直接吃分类监督
    """
    def __init__(
        self,
        pretrained: bool = True,
        ord2seq_d_model: int = 256,
        ord2seq_layers: int = 2,
        gate_hidden: int = 128,
...
def main(a):
    ...
    model = ResNet18SoftmaxOrdPlusBoundaryV2(
        pretrained=True,
    ...
    for epoch in range(a.epochs):
        ...
        for imgs, labels in tr_loader:
            ...
            expert_mask = valid_0A & (
                (candidate_mask.squeeze(1) > 0.5) | (hardness > a.expert_hard_thr)
            )
...
if __name__ == "__main__":
    ...
    ap.add_argument("--out", type=str, default="softmax_ordplus_o2s_boundary_v2.pt")
