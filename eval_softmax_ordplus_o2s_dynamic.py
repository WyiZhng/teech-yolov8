...
from train_softmax_ordplus_o2s_dynamic import ResNet18SoftmaxOrdPlusDynamic, Icdas4RoiDataset, map_ic4
...
def eval_model(model, loader, device, out_csv):
    ...
    all_p_soft, all_p_ord, all_alphas = [], [], []
...
def main():
    ...
    ap.add_argument("--out_val_csv", type=str, default="roi_val_dynamic.csv")
    ap.add_argument("--out_test_csv", type=str, default="roi_test_dynamic.csv")
    ap.add_argument("--bs", type=int, default=64)
    a = ap.parse_args()

    device = "cuda" if torch.cuda.is_available() else "cpu"
    ckpt = torch.load(a.ckpt, map_location="cpu")
    model = ResNet18SoftmaxOrdPlusDynamic(pretrained=False)
